 ...
 * Return:
 **********************************************************/
static void MDM_RTU_ReadFun(PModbus_RTU pModbus_RTU, uint8_t funCode, uint8_t slaveAddr,
 ...
 * Return:
 **********************************************************/
static void MDM_RTU_WriteSingleFun(PModbus_RTU pModbus_RTU, uint8_t funCode, uint8_t slaveAddr,
        uint16_t startAddr, uint16_t value) {
    MD_RTU_SEND_MODE(pModbus_RTU);
    MEM_RTU_START_EN(pModbus_RTU);
    MEM_RTU_EN_QUEUE(pModbus_RTU, slaveAddr);
    MEM_RTU_EN_QUEUE(pModbus_RTU, funCode);
 ...
 * Return:
 **********************************************************/
static void MDM_RTU_WriteFun(PModbus_RTU pModbus_RTU, uint8_t funCode, uint8_t slaveAddr,
        uint16_t startAddr, uint8_t numOf, uint8_t *data) {
    uint16_t i;
    uint8_t wLen = 0;
    MD_RTU_SEND_MODE(pModbus_RTU);
    MEM_RTU_START_EN(pModbus_RTU);
 ...
 * Return: true, false
 **********************************************************/
bool MDM_RTU_InsideWriteBits(void *obj, uint16_t modbusAddr, uint16_t numOf, uint8_t *bit,
    ...
    for (i = 0; i < MDM_REG_COIL_ITEM_NUM; i++) {
        ...
        if (pModbus_RTU->pMapTableList[i]->modbusAddr <= modbusAddr
                && (pModbus_RTU->pMapTableList[i]->modbusAddr
                        + pModbus_RTU->pMapTableList[i]->modbusDataSize) >= (modbusAddr + numOf)) {
            if (pModbus_RTU->pMapTableList[i]->addrType == opAddrType) {
                ...
                for (j = 0; j < numOf; j++) {
                    if (MD_GET_BIT(bit[j >> 3], j % 8)) {
                        MD_SET_BIT(
                                MDS_RTU_REG_COIL_ITEM_DATA( pModbus_RTU->pMapTableList[i])[(offsetAddr+j)>>4],
                                (j + offsetAddr) % 16);
                    } else {
                        MD_CLR_BIT(
                                MDS_RTU_REG_COIL_ITEM_DATA( pModbus_RTU->pMapTableList[i])[(offsetAddr+j)>>4],
 ...
 * Return: true, false
 **********************************************************/
bool MDM_RTU_InsideWriteRegs(void *obj, uint16_t modbusAddr, uint16_t numOf, uint16_t *reg,
        uint8_t isBigE, AddrType opAddrType, uint8_t devAddr) {
    uint16_t i;
    PModbus_RTU pModbus_RTU = obj;
    if (pModbus_RTU == NULL) {
        return false;
...
MDError MDM_RTU_NB_RW(PModbus_RTU_CB pModbus_RTU_CB, ModbusFunCode funCode, uint8_t slaveAddr,
    ...
    if (pModbus_RTU_CB->sendFlag == 0) {
        ...
        if (funCode >= 1 && funCode <= 4) {
            ...
            MDM_RTU_WriteFun(pModbus_RTU_CB->pModbus_RTU, funCode, slaveAddr, startAddr, numOf,
                    (uint8_t*) (wData));
        }
        ...
        if (pModbus_RTU_CB->pModbus_RTU->recvFlag) {
            ...
            case 0x2: {
                ...
                for (i = 0; i < byte; i++) {
                    ...
                    if (!MDM_RTU_InsideWriteBits(pModbus_RTU_CB->pModbus_RTU, wAddr,
                            ((index < 8) ? index : 8), &rByte, (AddrType) funCodeByte, slaveAddr)) {
...
//                    pModbus_RTU_CB->pModbus_RTU->parentObj=NULL;
            ...
            case 0x4: {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    MDdeQueue(&(pModbus_RTU_CB->pModbus_RTU->mdSqQueue), &rByte);
                    wTemp |= rByte;
                    if (!MDM_RTU_InsideWriteRegs(pModbus_RTU_CB->pModbus_RTU, startAddr + i, 1,
                            &wTemp, 0, (AddrType) funCodeByte, slaveAddr)) {
                        errRes = ERR_DATA_SAVE;
                        goto _exit;
                    }
                }
...
MDError MDM_RTU_NB_WriteSingleCoil(PModbus_RTU_CB pModbus_RTU_CB, uint8_t slaveAddr,
    ...
    uint16_t temp;
    temp = boolVal ? 0xFF00 : 0x0000;
    return MDM_RTU_NB_RW(pModbus_RTU_CB, WRITE_SIN_COIL_MDM, slaveAddr, startAddr, 1,
            (void*) (&temp));
}
...
MDError MDM_RTU_NB_WriteCoils(PModbus_RTU_CB pModbus_RTU_CB, uint8_t slaveAddr, uint16_t startAddr,
        uint16_t numOf, uint8_t *val) {
    return MDM_RTU_NB_RW(pModbus_RTU_CB, WRITE_COILS_MDM, slaveAddr, startAddr, numOf,
            (void*) (val));
}
