...
typedef enum {
    ...
    MDM_READ_INPUT     = 2,
    MDM_READ_HOLD_REG  = 3,
    MDM_READ_INPUT_REG = 4,
    MDM_WRITE_SIN_COIL = 5,
    MDM_WRITE_SIN_REG  = 6,
    MDM_WRITE_COILS    = 15,
    MDM_WRITE_REGS     = 16
} ModbusFunCode;
...
typedef struct {
    ...
    PMapTableItem pMapTableList[MDM_REG_COIL_ITEM_NUM];  // Discrete mapping list, the data read is stored in it
...
#endif
    uint32 lastTimesTick;                                // The time of the last reception, 0xFFFFFFF means that the detection frame has not started
    ...
    void *parentObj;                                     // Parent class address, used to distinguish time slice occupation during non-blocking read
    uint8 recvFlag;                                      // 1 means one frame of data is received
...
typedef struct {
    ...
    uint8 flag;               // bit0:Device offline polling flag (when flag is 1, the control block will not be polled when the device is offline).
...
void MDM_RTU_CB_Init(PModbus_RTU_CB pModbusRTUCB, PModbus_RTU pModbusRTU, uint32 sendIntervalTime, uint32 sendOverTime// Send timeout
