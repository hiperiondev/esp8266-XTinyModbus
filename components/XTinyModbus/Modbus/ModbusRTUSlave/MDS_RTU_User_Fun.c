...
BOOL MDS_RTU_ReadBits(void *obj, uint16 modbusAddr, uint16 numOf, uint8 *res, AddrType opAddrType) {
    ...
    for (i = 0; i < MDS_REG_COIL_ITEM_NUM; i++) {
        ...
        if (pModbusS_RTU->pMapTableList[i]->modbusAddr <= modbusAddr
                && (pModbusS_RTU->pMapTableList[i]->modbusAddr + pModbusS_RTU->pMapTableList[i]->modbusDataSize)
                        >= (modbusAddr + numOf)) {
            if (pModbusS_RTU->pMapTableList[i]->addrType == opAddrType) { // Specified type
                ...
                for (j = offsetAddr; j < offsetAddr + numOf; j++) {
                    if (MD_GET_BIT(MDS_RTU_REG_COIL_ITEM_DATA(pModbusS_RTU->pMapTableList[i])[j>>4], j % 16)) {
    ...
    for (i = 0; i < MDS_REG_COIL_ITEM_NUM; i++) {
        ...
        if (pModbusS_RTU->pMapTableList[i]->modbusAddr <= modbusAddr
            ...
            if (pModbusS_RTU->pMapTableList[i]->addrType == opAddrType) {
                uint16 offsetAddr = modbusAddr - MDS_RTU_REG_COIL_ITEM_ADDR(pModbusS_RTU->pMapTableList[i]);
                if (bit) {
                    MD_SET_BIT(MDS_RTU_REG_COIL_ITEM_DATA(pModbusS_RTU->pMapTableList[i])[offsetAddr>>4],
                            offsetAddr % 16);
                } else {
                    MD_CLR_BIT(MDS_RTU_REG_COIL_ITEM_DATA(pModbusS_RTU->pMapTableList[i])[offsetAddr>>4],
                            offsetAddr % 16);
                }
                return TRUE;
            }
            ...
            if (pModbusS_RTU->pMapTableList[i]->addrType == opAddrType) { // Must be BIT type
                ...
                for (j = 0; j < numOf; j++) {
                    ...
                    if (MD_GET_BIT(byteData[j >> 3], j % 8)) {
                        MD_SET_BIT(MDS_RTU_REG_COIL_ITEM_DATA( pModbusS_RTU->pMapTableList[i])[(offsetAddr+j)>>4],
                                (j + offsetAddr) % 16);
                    } else {
                        MD_CLR_BIT(MDS_RTU_REG_COIL_ITEM_DATA( pModbusS_RTU->pMapTableList[i])[(offsetAddr+j)>>4],
                                (j + offsetAddr) % 16);
                    }
                }
                return TRUE;
