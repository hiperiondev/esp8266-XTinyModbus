...
uint8 MDS_RTU_WriteDataProcess(PModbusS_RTU pModbus_RTU, uint16 reg, uint16 regNum, uint8 funCode, uint16 *data,
        uint8 byteCount);
/*********************************END******************************************/
...
void MDS_RTU_Init(PModbusS_RTU pModbusRTU, MD_RTU_SerialInit mdRTUSerialInitFun, uint8 salveAddr, uint32 baud,
        uint8 dataBits, uint8 stopBits, uint8 parity) {
    uint8 i;
    float T;
    if (pModbusRTU == NULL) {
        return;
    ...
    MDS_RTU_FUN_CODE(pModbus_RTU) == 16) {
        ...
        if (MDS_RTU_FUN_CODE(pModbus_RTU) == 15) {
            if (((regNum >> 3) + ((regNum % 8) ? 1 : 0)) != bytesNum) {
                MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_COILS, ILLEGAL_DAT_VAL);
                // A length mismatch error occurred, discard it
                goto __exit0;
            }
        } else { //16
            if ((regNum << 1) != bytesNum) {
                MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_REGS, ILLEGAL_DAT_VAL);
        ...
        if (pModbus_RTU->pMapTableList[i]->modbusAddr <= reg
            ...
            if ((funCode == 1 && pModbus_RTU->pMapTableList[i]->addrType == COILS_TYPE)
                ...
                for (j = offsetAddr; j < offsetAddr + regNum; j++) {
                    ...
                    if (MD_GET_BIT(MDS_RTU_REG_COIL_ITEM_DATA(pModbus_RTU->pMapTableList[i])[j>>4], j % 16)) {
...
uint8 MDS_RTU_WriteDataProcess(PModbusS_RTU pModbus_RTU, uint16 reg, uint16 regNum, uint8 funCode, uint16 *data,
    ...
    case 5: // Write single coil
    {
        if (data[0] == 0xFF00 || data[0] == 0x0000) {
            ...
            if (res) {
                ...
                MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_SIN_COIL, ILLEGAL_DAT_ADDR);
            }
        } else {
            MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_SIN_COIL, ILLEGAL_DAT_VAL);
        }
    }
        break;
    case 15: // Write multiple coils
        ...
        if (res) {
            ...
            MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_COILS, ILLEGAL_DAT_ADDR);
        }
        break;
    case 6: // Write single register
        ...
        if (res) {
            ...
            MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_SIN_REG, ILLEGAL_DAT_ADDR);
        }
        break;
    case 16: // Write multiple registers
        ...
        if (res) {
            ...
            MDS_RTU_SendErrorCode(pModbus_RTU, MDS_WRITE_REGS, ILLEGAL_DAT_ADDR);
        }
        break;
    }
    if (!res) {
